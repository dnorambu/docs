...
Engine analyzes both trace content and run feedback when selecting and ranking traces for each scan. Feedback — including online evaluator scores, annotation queue scores, and user feedback submitted via the SDK — is treated as a high-priority signal, not supplementary data.

Specifically, Engine:

- Reads the feedback keys present in your project and performs a dedicated pull of low-scoring traces for each key, so traces with poor evaluator scores are guaranteed to be included in the sample rather than left to recency.
...
Any source that writes feedback to a run — online evaluators, annotation queues, or SDK-logged scores — contributes to this prioritization automatically. No special setup is required beyond having evaluators or annotation queues in place.
...
<Note>
**Trace Scope limitation:** The scope filter only accepts run name and metadata conditions. You cannot scope Engine's scan by feedback key, evaluator name, or score threshold. If you want Engine to focus on traces associated with a specific evaluator's low scores, use [**Preferences**](#configure-engine) to tell Engine what to prioritize, or review the [**Agent Overview**](#configure-engine) document to steer Engine's analysis toward a particular concern. Engine already factors in all feedback signals automatically — see [How Engine samples and prioritizes traces](#how-engine-samples-and-prioritizes-traces).
</Note>
...
- **Focus on specific traces**: Narrow Engine's attention to a subset of runs by run name or metadata. Edits save automatically and take effect on the next scan. Note that scope conditions are limited to run name and metadata — you cannot filter by feedback key or score. See [Focus on specific traces](#focus-on-specific-traces).
