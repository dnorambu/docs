...
LangChain upgrades the LangSmith version in your data plane once a week. Upgrades are rolling, replica-by-replica (make-before-break), so there is no whole-service downtime.
...
You can deploy observability tooling into the cluster, such as a Datadog agent or an OpenTelemetry collector, as long as it does not affect the operation of LangSmith.
