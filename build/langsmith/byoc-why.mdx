...
The core value is simple: keep the data plane in your AWS account, while LangChain handles managing the LangSmith infrastructure.
...
- **SaaS-aligned architecture**: the data plane is provisioned using an architecture that is tried and tested.
...
You can move to BYOC from an existing self-hosted deployment. <b>Traces are not migrated today</b>, but users, roles, datasets, experiments, prompts, annotation queue configuration, automation rules, and dashboards can be copied over.
