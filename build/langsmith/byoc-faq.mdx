...
<Accordion title="Do I need a dedicated AWS account?">
    We recommend a fresh account for clearer billing, and because some permissions cannot be scoped to specific resources. But it is <b>not required</b>.
</Accordion>

<Accordion title="Can I install BYOC into an existing VPC?">
<b>Not today.</b> LangChain provisions a dedicated VPC for each data plane. Support for deploying into an existing VPC is planned.
</Accordion>
...
End-to-end provisioning of a data plane takes around <b>60 to 90 minutes</b>. Provisioning time can be vary due to the inconsistent nature of AWS resource provisioning.
</Accordion>
...
If you plan to use VPC peering, select a range that does not overlap with the VPC you intend to peer with. <b>The CIDR range cannot be changed after provisioning.</b>
</Accordion>
...
<b>Not today.</b> Data is encrypted at rest with cloud provider managed keys. Support for customer-managed encryption keys (CMK) is planned.
</Accordion>
...
<b>Not today.</b> Container images are pulled from the LangChain AWS account over a VPC endpoint. Support for private registries is on the roadmap.
</Accordion>
